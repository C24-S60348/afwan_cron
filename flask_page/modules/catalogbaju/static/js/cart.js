...
export function getGuestId() {
  ...
  } catch (_) { state.guestId = "guest_" + Date.now(); }
...
export function loadGuestCarts() {
  ...
  } catch (_) { state.guestCarts = {}; }
}

export function saveGuestCarts() {
  try { localStorage.setItem(STORAGE_KEY_GUEST_CARTS, JSON.stringify(state.guestCarts)); } catch (_) {}
...
export function renderCartModal(openItemModalByItem) {
  ...
  if (sendBtn) { sendBtn.disabled = submitted; sendBtn.textContent = submitted ? "Sent to owner" : "Send to owner"; }
  state.cart.forEach((entry, index) => {
    ...
    li.innerHTML = `
      <div class="cart-modal__item-img-wrap"><img class="cart-modal__item-img" src="${escapeAttr(imgSrc)}" alt="" /></div>
    ...
    `;
    li.querySelector(".cart-modal__item-remove").addEventListener("click", () => {
...
export function initCartModal(openItemModalByItem, renderCartModalFn) {
  ...
  function close() { modal.classList.remove("cart-modal--open"); modal.setAttribute("aria-hidden", "true"); }
...
export function initGuestCartModal() {
  ...
  if (!modal) return;
  function close() { modal.classList.remove("guest-cart-modal--open"); modal.setAttribute("aria-hidden", "true"); }
  closeBtn?.addEventListener("click", close);
  backdrop?.addEventListener("click", close);
}
