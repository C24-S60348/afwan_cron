...
export function getItemImages(item) {
  ...
  if (single) {
    if (!list.includes(single)) list.unshift(single);
      ...
      if (idx > 0) { list.splice(idx, 1); list.unshift(single); }
    }
  }
...
export function getTagColor(category) {
  ...
  return state.appData.tags[category.toLowerCase()] || "";
}
...
export function createCatalogItemElement(item) {
  ...
  const tagStyle = tagColor
    ? ` style="background:${escapeAttr(tagColor)}20; border-color:${escapeAttr(tagColor)}; color:${escapeAttr(tagColor)};"`
    : "";
  el.innerHTML = `
...
export function saveCatalogToStorage() {
  try { localStorage.setItem(STORAGE_KEY_CATALOG_ITEMS, JSON.stringify(state.allItems)); } catch (_) {}
...
export function buildCatalogFromData(bindItemClicks) {
  ...
  for (const pageItems of pages) {
    ...
    for (let i = 0; i < ITEMS_PER_PAGE; i++) {
      ...
      if (i < pageItems.length) cell.appendChild(createCatalogItemElement(pageItems[i]));
      else cell.classList.add("grid-cell--empty");
...
export function renderOwnerCatalogList() {
  ...
  state.allItems.forEach((item, index) => {
    ...
    `;
    li.querySelector(".owner-panel__item-remove")?.addEventListener("click", () => {
      state.allItems.splice(index, 1);
      if (state.catalogData) state.catalogData.items = state.allItems;
      saveCatalogToStorage();
      state.onCatalogChange?.();
    });
    li.querySelector(".owner-panel__item-edit")?.addEventListener("click", () => {
      ...
      const imagesInput = prompt("Up to 3 image URLs/filenames (comma-separated)", currentImages.join(", "));
      if (imagesInput != null) {
        const nextImages = imagesInput.split(",").map((s) => s.trim()).filter(Boolean).slice(0, 3);
        if (nextImages.length > 0) { item.images = nextImages; item.image = nextImages[0]; }
      }
