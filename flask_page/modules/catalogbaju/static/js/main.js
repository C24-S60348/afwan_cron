...
import { buildCatalogFromData, createDots, updateDots, loadCatalogFromStorage, renderOwnerCatalogList, getTagColor } from "./catalog.js";
import { initSlider, initTabs, initSliderAndBuildCatalog, updateSlidePosition, bindItemClicks, goToPage } from "./slider.js";
import { getGuestId, loadGuestCarts, updateCartCount, renderCartModal, initCartModal } from "./cart.js";
import { openItemModal, openItemModalByItem, initItemModal } from "./itemModal.js";
...
function applyTabColors() {
  document.querySelectorAll(".catalog-tab").forEach((tab) => {
    ...
    if (color) { tab.style.background = `${color}20`; tab.style.borderColor = color; tab.style.color = color; }
...
function initInfoModal() {
  ...
  function open() {
    ...
    modal.classList.add("info-modal--open"); modal.setAttribute("aria-hidden", "false");
  }
  function close() { modal.classList.remove("info-modal--open"); modal.setAttribute("aria-hidden", "true"); }
...
async function loadCatalog() {
  ...
  try {
    ...
    state.defaultCatalogItems = Array.isArray(data?.catalog?.items) ? JSON.parse(JSON.stringify(data.catalog.items)) : [];
    state.resetPassword = String(data?.catalog?.resetPassword || data?.resetPassword || "catalogbaju-reset");
    ...
    if (state.catalogData?.items?.length) initCatalog();
    else if (track) track.innerHTML = "<p style='padding:2rem;color:var(--text-soft)'>No catalog data. Check appsettings.json.</p>";
    updateCartCount();
    applyBrand();
  } catch (err) {
    console.error("Failed to load appsettings.json", err);
    if (track) track.innerHTML = "<p style='padding:2rem;color:var(--text-soft)'>Failed to load catalog.</p>";
  }
}
