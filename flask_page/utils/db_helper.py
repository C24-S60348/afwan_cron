...
DATABASE LOCATION
    All database files live in:  afwan_cron/db/<appname>.db
    This folder is GITIGNORED (see .gitignore: db/).  It is created
    automatically at runtime — do NOT commit .db files.
...
STEP 1 — In your blueprint file (e.g. flask_page/modules/myapp/myapp.py)
─────────────────────────────────────────────────────────────────────────────

    from flask_page.utils.db_helper import get_connection, ensure_tables

    _APP_NAME = 'myapp'   # becomes myapp.db in afwan_cron/db/

    # Table name follows convention: projectname_tablename_db
    _T_THINGS = 'myapp_things_db'
    ...
    def init_myapp_db():
        ...
        # Optionally seed defaults here if tables are empty:
        # _seed_myapp_if_empty()


STEP 2 — Export init function from your module's __init__.py
─────────────────────────────────────────────────────────────────────────────

    # flask_page/modules/myapp/__init__.py
...
─────────────────────────────────────────────────────────────────────────────
    ...
    # Add to the startup block at the bottom:
...
STEP 4 — Use get_connection() in your routes
─────────────────────────────────────────────────────────────────────────────
